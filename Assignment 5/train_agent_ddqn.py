...
for step in range(steps):
	...
	if step == 700000:
		...
		with open("dd600k.json", "w") as json_file:
			json_file.write(model_json)
		# serialize weights to HDF5
		model_q.save_weights("dd600k.h5")
		print("Saved model-600k to disk")

	if step == 800000:
		model_json = model_q.to_json()
		with open("dd800k.json", "w") as json_file:
			json_file.write(model_json)
		# serialize weights to HDF5
		model_q.save_weights("dd800k.h5")
		print("tarSaved model-800k to disk")

	if step == 1000000:
		model_json = model_q.to_json()
		with open("dd_1000k.json", "w") as json_file:
			json_file.write(model_json)
		# serialize weights to HDF5
		model_q.save_weights("dd_1000k.h5")
		print("Saved model-1000k to disk")
		
		# saver.save(sess, dir + '/data-all.chkp')
		with open('dd_loss', 'wb') as ls:
			pickle.dump(ll, ls)
		with open('dd_reward', 'wb') as rd:
			pickle.dump(re, rd)
		with open('dd_term', 'wb') as tr:
			pickle.dump(term, tr)
		with open('dd_step', 'wb') as ts:
			pickle.dump(st, ts)	
		with open('dd_no_error', 'wb') as er:
			pickle.dump(erro, er)						
#Used for Testing
